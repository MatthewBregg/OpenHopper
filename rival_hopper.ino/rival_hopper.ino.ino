...
const int pusher_c = A3;
void set_pusher(bool on) {
  analogWrite(PWM, 255.0 * get_motor_speed_factor(13));
  digitalWrite(LED_BUILTIN, on);
  if ( on ) {
    
    digitalWrite(pusher_c, LOW);
    pinMode(pusher_c, INPUT);
  } else {
    digitalWrite(pusher_c, LOW);
    pinMode(pusher_c, OUTPUT);
  }
}
...
void loop(){
  if(firstRun) {
    ...
    while(gov_update_repeats) {
      ...
      // Ideas to lower this number:
      // -- Increase crush
      // -- Print lighter (ASA, And possible lower infill/layers very carefully!).
  ...
  if(currTrigState && prevTrigState){
    ...
    set_spinner(true); // Start the hopper spinner.
  ...
  while(readTrigger()) { delay(2); }
    ...
    drive1TachValid = false;
    constexpr int spinnerShutdownTime = 500; 
    if((millis() - lastTriggerUp)>spinnerShutdownTime){
      // turn spinner motor off
      set_spinner(false);
    }
  }
}
